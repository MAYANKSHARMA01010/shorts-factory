...
> **Fix the paths.** Scripts contain absolute paths like `C:/Dikshant/Money making/`. Search-and-
...
The publish scripts (`scripts/publishing/post_to_youtube.py`, `scripts/analytics/yt_analytics.py`) read the YouTube client id/secret from
...
Enable it once for your GCP project so `scripts/analytics/yt_analytics.py` can read CTR / retention:
