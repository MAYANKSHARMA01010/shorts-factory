...
- NEVER edit the hard rules or anything OUTSIDE the `<!-- LEARNED-RULES-START -->` / `END` markers in
  SKILL.md. Back up SKILL.md first: `bash scripts/runners/bash/backup_skill.sh`.
...
- Run `python scripts/analytics/yt_analytics.py`; read `data/analytics/performance_latest.md`.
...
- Read `pipeline/intelligence/learnings.md` (prior observations/experiments — check which are due for review today) and
  `pipeline/ledgers/daily_posts_ledger.md` (what was posted + its topic/niche/title/hook).
...
6. **TOPIC / NICHE (biggest lever).** Rank videos by **impressions-velocity × AVP (NOT raw views)** —
   ...
   `pipeline/ledgers/daily_topics.md` ordering. (Respect the SKILL.md niche-mix rule / owner decisions.)
...
## STEP 5 — Apply (safely)
1. `bash scripts/runners/bash/backup_skill.sh`.
...
3. **daily_topics.md** — reprioritize: move winning-pattern topics up, mark losers, append fresh
   researched topics (keep ≥15 unused). Respect the current niche-mix rule in SKILL.md (in `pipeline/ledgers/daily_topics.md`).
4. **learnings.md** — append this run's full analysis (observations + evidence + experiments started +
   results of experiments that came due) to `pipeline/intelligence/learnings.md`. Append-only; mark superseded entries, don't delete.
5. If a change materially affects production, update `pipeline/prompts/daily_shorts_prompt.md` conservatively.
...
posting cadence, drop a format), DO NOT auto-apply it — append it to `docs/DECISIONS_FOR_OWNER.md` (the
...
could NOT read analytics or failed its core job, send a best-effort alert: `python "C:/Dikshant/Money
