...
2. Channel connected? `python ".claude/skills/ultimate-short/post_to_postiz.py" --list` (needs a `youtube`
   ...
   `C:\Dikshant\auto posting tool\postiz-app-main\Videos to post\`, log "needs manual post", stop.
3. Read `pipeline/ledgers/daily_posts_ledger.md` — next slug number = (last NNN)+1; never repeat a used topic/title.
4. **Read the ★ SELF-LEARNED RULES** section of `.claude/skills/ultimate-short/SKILL.md` (between the
   ...
   grows views.)
4b. **Read `pipeline/intelligence/competitor_playbook.md` → § TECHNIQUE CATALOG** and apply its top techniques (hook shapes,
...
5. Read `pipeline/ledgers/daily_topics.md` — take the TOP UNUSED topic that best fits the current learned niche-mix rule.
...
`pipeline/ledgers/variation_ledger.md` and choose a DISTINCT combination:
- **title-shape** from `pipeline/variation/title_shapes.md` — not used in the last 6 rows (and vary the TYPE).
- **format** from `pipeline/variation/formats.md` — not within the last 5 (this sets the script skeleton).
- **visual-skin** from `pipeline/variation/visual_skins.md` — not within the last 4 (different palette/hero/motion).
- **voice + length + pace** from `pipeline/variation/voices.md` — voice not within the last 4.
...
`data/descriptions/daily_desc_<slug>.txt`, and tags. **Publish DIRECTLY to YouTube — the RELIABLE path.** (Postiz's
...
```
python scripts/publishing/post_to_youtube.py --video "<final.mp4>" \
  --title "<title>" --description @"data/descriptions/daily_desc_<slug>.txt" --tags "<a,b,c,...>" --privacy public
```
...
`python scripts/publishing/notify_email.py --subject "⚠ Shorts: <slug> did not publish" --body "<the reason + that the mp4 is staged for manual posting>"`.
...
## FINISH
Append a row to `pipeline/ledgers/daily_posts_ledger.md`: date | time | slug | title | topic # | the youtube URL | final
mp4 path | GATE verdict. Mark the topic USED in `pipeline/ledgers/daily_topics.md`. **Append a row to `pipeline/ledgers/variation_ledger.md`**
