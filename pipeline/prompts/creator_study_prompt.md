...
`pipeline/intelligence/competitor_playbook.md` so every future video is more engaging. This is the channel's fastest growth
...
- Read `pipeline/ledgers/studied_videos.md` (never re-study a video) and `pipeline/intelligence/competitor_playbook.md` (what we already know).
- Choose 1–2 topic queries to study this run, rotating across our clusters + the next planned
  `pipeline/ledgers/daily_topics.md` topics + whatever our analytics say is winning. Vary creators (don't only study one).
- Find the real winners: `python scripts/analytics/yt_top.py "<query>" --n 8`
  (returns the top Shorts by views + urls; also saved to `data/analytics/top_<slug>.json`).

## STEP 2 — WATCH the winners (deep study, not skim)
For **2–3 NEW top videos** (highest views, not already in `pipeline/ledgers/studied_videos.md`; prefer variety of creators):
...
- Append each full breakdown to `pipeline/intelligence/competitor_playbook.md` § STUDY LOG (creator · views · url · breakdown).
...
- Append a row per studied video to `pipeline/ledgers/studied_videos.md`.

## STEP 4 — Compare to US (find the concrete gap)
- Run/read `python scripts/analytics/yt_analytics.py` → `data/analytics/performance_latest.md`.
...
- `bash scripts/runners/bash/backup_skill.sh` first.
...
- If a finding implies a strategic change (niche/cadence), append to `docs/DECISIONS_FOR_OWNER.md`, don't auto-apply.

## GUARDRAILS
- Learn TECHNIQUE, never copy content/scripts (stay original + TRUE + brand-safe).
- Study NEW videos each run (dedup via `pipeline/ledgers/studied_videos.md`); broaden creators + niches over time.
...
## FINISH
Append one line to `runtime/logs/study_run.log` (date · queries · videos studied · top new technique). Print a short
summary and stop.
summary: what you watched, the biggest technique added, and the top gap vs our videos. If the run could
NOT study any video (yt_top / `/watch` errored), send a best-effort alert: `python "C:/Dikshant/Money
