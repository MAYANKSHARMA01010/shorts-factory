...
if "%c%"=="5" ( echo.& echo    Sending digest email...& docker start postiz postiz-postgres >nul 2>&1 & python scripts\publishing\daily_digest.py & echo.& pause & goto menu )
if "%c%"=="6" ( echo.& echo    Tailing daily_run.log ... press Ctrl+C to stop.& echo.& powershell -NoProfile -Command "Get-Content '%ROOT%\runtime\logs\daily_run.log' -Tail 30 -Wait" & goto menu )
