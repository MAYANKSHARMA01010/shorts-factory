...
def run_task(names):
    mac_scripts = {
        "DailyShorts": "scripts/runners/bash/daily_shorts.sh",
        "CreatorStudy": "scripts/runners/bash/study_creators.sh",
        "ShortsLearn": "scripts/runners/bash/learn_shorts.sh",
        "ShortsDigest": "scripts/runners/bash/digest.sh"
    }
    for n in names:
        if n in mac_scripts:
            script_path = str(PROJECT_ROOT / mac_scripts[n])
            # Run the script in the background detached from the dashboard
            subprocess.Popen(["bash", script_path], start_new_session=True, cwd=str(PROJECT_ROOT))
...
def read(p):
    q = PROJECT_ROOT / p if isinstance(p, (str, Path)) else p
...
def analytics():
    p = PROJECT_ROOT / "data" / "analytics" / "performance_latest.json"
    if not p.exists():
        return None
    try:
...
def pending_decision():
    d = read("docs/DECISIONS_FOR_OWNER.md")
...
.banner{{background:#fff;border:1px solid #fff;color:#000;padding:6px 10px;margin:0 0 10px;font-weight:bold}}
    ...
    if running:
        for t in running:
            log = {"DailyShorts": "runtime/logs/daily_run.log", "CreatorStudy": "runtime/logs/study_run.log",
                   "ShortsLearn": "runtime/logs/learn_run.log", "ShortsDigest": "runtime/logs/digest_run.log"}.get(t["name"], "")
    ...
    real_published = len(ledger_recent("pipeline/ledgers/daily_posts_ledger.md", 999))
    ...
    tools = [("make_narration.py", "packages/ClipPilot/remotion_explainer/make_narration.py", "TTS narration + word timing"),
             ("post_to_youtube.py", "scripts/publishing/post_to_youtube.py", "direct YouTube upload (publish)"),
             ("yt_analytics.py", "scripts/analytics/yt_analytics.py", "channel/video analytics"),
             ("yt_top.py", "scripts/analytics/yt_top.py", "find top competitor Shorts"),
             ("recut.py", ".claude/skills/punchy-recut/recut.py", "pacing + SFX finish"),
             ("notify_email.py", "scripts/publishing/notify_email.py", "email digest/alerts"),
             ("variation/", "pipeline/variation/title_shapes.md", "anti-repetition engine")]
    ...
    for cells in ledger_recent("pipeline/ledgers/daily_posts_ledger.md", 6):
        if len(cells) >= 5:
            vid = vid_id(cells[4])
    ...
    st_rows = ledger_recent("pipeline/ledgers/studied_videos.md", 4)
    if st_rows:
    ...
    # DECISIONS + PIPELINE
    unused = len(re.findall(r"\| unused \|", read("pipeline/ledgers/daily_topics.md")))
    rules = len(re.findall(r"^- \*\*\[", read(".claude/skills/ultimate-short/SKILL.md"), re.M))
    var_last = ledger_recent("pipeline/ledgers/variation_ledger.md", 1)
