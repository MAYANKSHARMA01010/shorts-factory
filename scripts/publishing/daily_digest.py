...
def main():
    # 1) fresh analytics
    try:
        subprocess.run([sys.executable, str(PROJECT_ROOT / "scripts" / "analytics" / "yt_analytics.py")], capture_output=True, text=True, timeout=180)
    except Exception:
        pass
    perf = {}
    pj = PROJECT_ROOT / "data" / "analytics" / "performance_latest.json"
    if pj.exists():
    ...
    posts = []
    for line in read("pipeline/ledgers/daily_posts_ledger.md").splitlines():
    ...
    # 3) studied today
    studied = []
    for line in read("pipeline/ledgers/studied_videos.md").splitlines():
        if line.startswith(f"| {TODAY} "):
            cells = [c.strip() for c in line.strip().strip("|").split("|")]
            if len(cells) >= 7:
    ...
    decisions = read("docs/DECISIONS_FOR_OWNER.md")
    ...
    html_body = "".join(H)
    (PROJECT_ROOT / "runtime" / "state" / "digest_last.html").write_text(html_body, encoding="utf-8")  # always saved, even if email fails
    try:
